...
def build_model_dot():
    """Build a Graphviz DOT string describing the model pipeline."""
    ...
    edges = []
    # Input
    input_node = '"Input Image"'

    # PatchCore
    pc_node = '"PatchCore (Phase 1)"' if pc_path.exists() else None
    if pc_node:
        edges.append((input_node, pc_node))

    # YOLO
    if yolo_onnx.exists():
        yolo_node = '"YOLO (ONNX - Phase 2)"'
        edges.append((input_node, yolo_node))
    elif yolo_pt.exists():
    ...
    # Ensemble and decision
    ensemble_node = '"Ensemble Inspector"'
    if pc_node:
        edges.append((pc_node, ensemble_node))
    if yolo_node:
        edges.append((yolo_node, ensemble_node))
    edges.append((ensemble_node, '"Decision (Pass/Fail)"'))

    lines = ["digraph G {", "  rankdir=LR;", "  node [shape=box, style=filled, fillcolor=\"#eef3f8\"];"
            ]
    for a, b in edges:
        lines.append(f"  {a} -> {b};")
    lines.append("}")
...
class PCBHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/':
            ...
            self.path = '/webcam_interface.html'
            return super().do_GET()
        elif self.path == '/model_graph':
            return self.handle_model_graph()

        return super().do_GET()

    def handle_model_graph(self):
        """Return the model pipeline graph as an SVG (if graphviz available) or DOT text."""
        try:
            dot = build_model_dot()

            # Try to render SVG using graphviz Python package
            try:
                import graphviz
                ...
                # graphviz not installed or rendering failed; fall back to DOT text
                pass

            # Fallback: return DOT plain text
            self.send_response(200)
            self.send_header('Content-Type', 'text/plain; charset=utf-8')
            self.send_header('Access-Control-Allow-Origin', '*')
            self.end_headers()
            self.wfile.write(dot.encode('utf-8'))

        except Exception as e:
            self.send_error(500, f"Failed to build model graph: {e}")
